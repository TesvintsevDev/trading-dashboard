...
The API uses Node's built-in `node:sqlite` module and stores local data in `storage/trading-dashboard.sqlite`. The `storage/` folder is in the repo root `.gitignore`.
